...
The \Rfunarg{F-B} method uses a forward-backward algorithm described in \citet{Guedon2003}, 
...
package \Rpackage{tillingArray} \citealp{Hubertiling} is implemented and extended to handle Poison and Negative binomial distributed data. 
...
Another option is to use modified Bayes information criterion \Rfunarg{penalty='mBIC'} \citealp{mbic2007}, 
...
As in the ENCODE guide \citealp{ENCODEguide}, the study identified the p53 isoform observed in K562 cells has a longer 3'UTR region than the isoform seen in the GM12878 cell line. 

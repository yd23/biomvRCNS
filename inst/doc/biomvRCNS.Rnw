...
\footnote{Simultaneous treatment within group is currently available for \Rfunarg{emis.type= 'mvnorm'} in \Rfunction{biomvRhsmm}, 
...
The function will then call C codes and estimate the most likely state sequence, with either \Rfunarg{cMethod='BandF'} or \Rfunarg{cMethod='Viterbi'}. 
The \Rfunarg{BandF} method uses a forward-backward algorithm described in \citep{Guedon2003}, 
which gives a smooth state sequences, whereas the Viterbi algorithm with \Rfunarg{cMethod='Viterbi'} 
will require appropriate prior for the emission and sojourn distribution to run properly.
...
\begin{center}
<<coriellHsmmplot, fig=TRUE, echo=FALSE>>=
...
package \Rpackage{tillingArray} \citep{Hubertiling} is implemented and extended to handle Poison and Negative binomial distributed data. 
...
will be estimated from \Rfunarg{xAnno} by calling function \Rfunction{sojournAnno}. 
When given a  \Rclass{TranscriptDb} object to \Rfunarg{xAnno}, state number would be set to 3 and each represents 'intergenic', 'intron', 'exon'.
...
As in the ENCODE guide \citep{ENCODEguide}, the study identified the p53 isoform observed in K562 cells has a longer 3'UTR region than the isoform seen in the GM12878 cell line. 
So here we plot our model estimates and consider the third state, namely 'exon', to represent detected transcripts. 
And the HSMM model clearly picked up the extra transcripts of the K562 cell line at the 3'UTR region. 
...
@
\begin{center}
<<plotENCODEHsmmK, fig=TRUE, echo=FALSE>>=
i<-mcols(rhsmm@res)[,'STATE']=='exon' & mcols(rhsmm@res)[,'SAMPLE']=='K562'
biomvRGviz(exprgr=cgr[,'K562'], gmgr=gmgr, 
  seggr=rhsmm@res[i], plotstrand='-', regionID='TP53', tofile=FALSE)
@ 
\end{center}
\begin{center}
<<plotENCODEHsmmG, fig=TRUE, echo=FALSE>>=
i<-mcols(rhsmm@res)[,'STATE']=='exon' & mcols(rhsmm@res)[,'SAMPLE']=='Gm12878'

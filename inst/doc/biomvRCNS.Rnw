...
rhsmm<-biomvRhsmm(x=xgr, maxbp=4E4, J=3, soj.type='gamma', 
	com.emis=F, emis.type='norm', prior.m='quantile', grp=c(1,2))
@ 
<<coriellHsmmres>>=
show(rhsmm)
@ 
...
The emission density could be estimated using all data or only data on the repective region or chromosome 
(identified by unique \Rfunction{seqnames}), controlling via \Rfunarg{com.emis}. 
...
obj<-biomvRGviz(exprgr=xgr[seqnames(xgr)=='chr1', 'Coriell.13330'], 
	seggr=rhsmm@res[mcols(rhsmm@res)[,'SAMPLE']=='Coriell.13330'], tofile=FALSE)
@ 
\end{center}
...
\begin{center}
<<plotdmr, fig=TRUE>>=
s<-seqnames(rhsmm@res) == 'chr1'
e<-seqnames(variosm) == 'chr1'
obj<-biomvRGviz(exprgr=variosm[e],  seggr=rhsmm@res[s], regionID='DMR', tofile=FALSE)
@ 
\end{center}

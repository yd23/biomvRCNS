...
In the above run, we limit the model complexity by setting the \Rfunarg{maxbp} to $5E4$, 
...
tmp<-biomvRhsmm(x=xgr[seqnames(xgr)=='chr1', 'Coriell.13330'], maxbp=5E4, J=3, 
	soj.type='gamma', emis.type='norm', maxit=1)
@


\begin{center}
<<fig=TRUE, echo=FALSE>>=
plot(tmp, tofile=FALSE)
@ 
\end{center}
...
calling function \Rfunction{sojournAnno}. The three states estimated would each represents 'intergenic', 'intron', 'exon'.

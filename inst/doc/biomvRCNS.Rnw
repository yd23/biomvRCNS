...
Additionally there is a built-in automatic grouping method, given a valid clustering method \Rfunarg{clusterm} and \Rfunarg{grp} set to \Robject{NULL}.
...
The \Rfunarg{F-B} method uses a forward-backward algorithm described in \citep{Guedon2003}, 

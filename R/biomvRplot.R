...
#				xAnno<-lapply(xAnno, function(z) z[start(z)>=min(start(xMeta)) & end(z)<=max(end(xMeta))])
#			} else {
...
#	nf<-layout(matrix(c(rep(nc+3,nc), rep(nc+4, 2), seq_len(nc+2)), nc+2, 2), heights=har, widths=war) 
